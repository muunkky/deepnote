...
- **Task Description:** Normalize _external_ integration ID matching to be case-insensitive, mirroring the lowercase convention already used for built-in IDs (`isBuiltinIntegration`) and for local-vs-required IDs in `packages/cli/src/integrations/fetch-and-merge-integrations.ts:23-24`.
