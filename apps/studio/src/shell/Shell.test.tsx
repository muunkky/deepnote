...
//
// `Shell` is the loaded-state view extracted from the old `App` (step 4): it takes the
// resolved `project` as a prop and owns selection ⇆ hash. `App` is now the fetch container
// that renders `Shell` once `loadProjectState` resolves (see App.test.tsx).
...
describe('Shell — notebook list', () => {
  it('renders one list entry per notebook in the fixture', () => {
    render(<Shell project={project} />)
